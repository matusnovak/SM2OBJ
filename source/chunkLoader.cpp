...
bool loadChunk(ffw::file* File, size_t FileOffset, int ChunkIndex, uint32_t ChunkData[][16][16], ffw::vec3i* Pos){
    ...
    if( ! (File->read(&timeStamp, sizeof(uint64_t)) &&
        ...
        ffw::logger().warning() << "Chunk error while reading chunk header!";
        return false;
    }
    ...
    if(relativePosition[0] > 9999 || relativePosition[0] < -9999 ||
       ...
       relativePosition[2] > 9999 || relativePosition[2] < -9999){
        ffw::logger().warning() << "Chunk error while reading chunk header! Wrong relative position!";
        return false;
    }
    ...
    if(ret != Z_OK){
        ffw::logger().warning() << "Chunk failed to load! Z-Lib init error!";
        inflateEnd(&strm);
        return false;
    }
    ...
    if(have != 12288){
        ffw::logger().warning() << "Can not decompress data! Chunk might be corrupted!";
        return false;
    }
