...
bool sm2obj::loadRawBlocks(const std::string& TempFolder, chunkBufferStruct* Input, int FileIndex){
    ...
    if(!input.open(TempFolder + "\\chunk-temp-processed-" + ffw::valToString(FileIndex) + ".raw", std::ios::binary | std::ios::in)){
        ffw::logError() << "Failed to load processed raw chunk data! Chunk index: " << FileIndex;
        return false;
    }
...
bool sm2obj::saveRawBlocks(const std::string& TempFolder, chunkBufferStruct* Input, int FileIndex){
    ...
    if(!output.open(TempFolder + "\\chunk-temp-processed-" + ffw::valToString(FileIndex) + ".raw", std::ios::out | std::ios::trunc | std::ios::binary)){
        ffw::logError() << "Failed to svae processed raw chunk data! Chunk index: " << FileIndex;
        return false;
    }
...
///=============================================================================
bool sm2obj::extractBlocks(const std::string& TempFolder, chunkBufferStruct* Input, int FileIndex, uint64_t* IndicesOffset, uint64_t* TexPosOffset, bool UseAtlas, bool ExportMaterials, bool ExportUV, const std::vector<blockInfoStruct>& BlockInfo){
    ffw::file vertices;
    ffw::file indices;

    if(!vertices.open(TempFolder + "\\chunk-temp-" + ffw::valToString(FileIndex) + ".vertices", std::ios::out | std::ios::trunc)){
        ffw::logError() << "Failed to svae chunk vertices! Chunk index: " << FileIndex;
        return false;
    }

    if(!indices.open(TempFolder + "\\chunk-temp-" + ffw::valToString(FileIndex) + ".indices", std::ios::out | std::ios::trunc)){
        ffw::logError() << "Failed to svae chunk indices! Chunk index: " << FileIndex;
        return false;
    }
    ...
    const sm2obj::blockInfoStruct* block = NULL;
    for(uint32_t i = 0; i < Input->indicesCount; i++){
        if(!UseAtlas && ExportMaterials && (lastId != Input->indicesMat[i][0] || lastTex != Input->indicesMat[i][1])){
            lastId = Input->indicesMat[i][0];
            lastTex = Input->indicesMat[i][1];
            block = findBlock(BlockInfo, lastId);
        ...
        if(UseAtlas && ExportMaterials && lastTex != Input->indicesMat[i][1]){
            lastTex = Input->indicesMat[i][1];
