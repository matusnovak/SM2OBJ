...
bool sm2obj::loadMeta(entityInfoStruct* Output, const std::string& Path, const std::string& Name){
    ffw::file input;
    if(!input.open(Path, std::ios::binary | std::ios::in)){
        return false;
    }

    ffw::logDebug() <<"Reading: " << Path;

    size_t pos = findName(&input, Name);

    if(pos == 0){
        ffw::logDebug() << "Could not load attachments for blueprint: " << Name << " Blueprint might be outdated or there are no attachments!";
        return true;
    }
	...
	for(uint32_t i = 0; i < totalEntries; i++){
		...
		if(i + 1 != totalEntries){
			...
			if(pos == 0){
				ffw::logWarning() << "Failed to read next attachment path from meta: " << Path;
