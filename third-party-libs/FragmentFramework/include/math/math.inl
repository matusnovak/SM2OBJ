...
inline ffw::mat4 ffw::makeProjectionMatrix(float FieldOfView, float Aspect, float Near, float Far){
    ...
    float tanFov = tanf(halfFov*DEG_TO_RAD);
    ffw::mat4 mat;
...
inline ffw::mat4 ffw::makeLookAtMatrix(ffw::vec3f Eyes, ffw::vec3f Target, ffw::vec3f UpVector){
    ffw::mat4 mat;
...
inline ffw::mat4 ffw::makeFpsCameraMatrix(ffw::vec3f Eyes, float Pitch, float Yaw){
    float cPitch = cos(Pitch*DEG_TO_RAD);
    float sPitch = sin(Pitch*DEG_TO_RAD);
    float cYaw = cos(Yaw*DEG_TO_RAD);
    float sYaw = sin(Yaw*DEG_TO_RAD);
...
inline float ffw::getAbsd(double Val){
...
inline float ffw::clampd(double Val){
